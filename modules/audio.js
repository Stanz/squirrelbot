...
bot.on('message', message => {
  ...
  if (matches && matches.length) {
    message.guild.channels.filter(channel => channel.type === 'voice').forEach(function(channel) {
      if (channel.members.map(member => member.user.id).indexOf(message.author.id) > -1) {
  ...
  if (voiceConnection) {
    ...
    if (matches && matches.length) {
        console.log('playing')
        let chosenFormat
        const stream = ytdl(matches[1], {filter : function(format) {
        ...
        message.reply('playing ['+chosenFormat.audioEncoding+'/'+chosenFormat.audioBitrate+'k]')
    }
