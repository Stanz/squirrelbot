      ...
      if (m) {
        ...
        if (m[1]) {
          ...
          switch (m[5].toLowerCase()) {
            case 'pt': tz = 'US/Pacific'
            break
            case 'et': tz = 'US/Eastern'
            break
            case 'ct': tz = 'US/Central'
            break
            case 'mt': tz = 'US/Mountain'
            break
            case 'pst': tz = 'Etc/GMT+8'
            break
            case 'pdt': tz = 'Etc/GMT+7'
            break
            case 'est': tz = 'Etc/GMT+5'
            break
            case 'edt': tz = 'Etc/GMT+4'
            break
            case 'mst': tz = 'Etc/GMT+7'
            break
            case 'mdt': tz = 'Etc/GMT+6'
            break
            case 'cst': tz = 'Etc/GMT+6'
            break
            case 'cdt': tz = 'Etc/GMT+5'
            break
            case 'cest': tz = 'Etc/GMT-2'
            break
            case 'cet': tz = 'Etc/GMT-1'
            break
            case 'gmt': tz = 'Etc/GMT'
            break
            case 'bst': tz = 'Etc/GMT-1'
            break
            case 'utc': tz = 'Etc/UTC'
            break
          }
          ...
          let obj = {
            ...
            month: now.month(),
            date: now.date(),
            hour: m[4] && m[4].toLowerCase() == 'pm' ? (parseInt(m[2])+12) : m[2]
          }
          //console.log(obj)
          //console.log(tz)
        ...
        messages.push(time.fromNow(true) + ' from now ' + convertTo.map(tz => '['+time.tz(tz).format("HH:mm") + ' ' + tz.split('/')[1]+']').join(' '))
      }
    })
    if (messages.length) {
